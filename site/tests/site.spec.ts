...
// The landing page auto-opens the waitlist modal once at ~55% scroll (a fresh-visitor
// behavior covered by the waitlist tests below). For every other test we model a
// RETURNING visitor so that one-time overlay never intercepts unrelated interactions.
test.describe('landing page (returning visitor)', () => {
  test.beforeEach(async ({ page }) => {
    await page.addInitScript(() => {
      try { localStorage.setItem('archify:waitlist', 'dismissed'); } catch {}
    });
  });
